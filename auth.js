...
async function initAuth(){
  ...
  if(window.noorProbeSupabase){
    ...
    if(!probe.ok){
      setAuthGate(false);
      const notice=qs('#dbNotice');if(notice){notice.classList.remove('hidden');notice.textContent=probe.message+' سيظل المحتوى العام متاحًا، بينما تسجيل الدخول والحفظ السحابي متوقفان مؤقتًا.'}
      return;
    }
  }
