...
async function noorProbeSupabase(ms=8000){
  ...
  try{
    const r=await fetch((CFG.supabaseUrl||'').replace(/\/$/,'')+'/auth/v1/health',{headers:{apikey:CFG.publishableKey},cache:'no-store',signal:ctrl.signal});
    clearTimeout(timer);
    if(r.status===401||r.status===403)return {ok:false,stage:'key',message:'تم الوصول إلى Supabase لكن المفتاح مرفوض.'};
    return {ok:true,status:r.status,message:'Supabase متاح.'};
  }catch(error){
    clearTimeout(timer);
    return {ok:false,stage:error?.name==='AbortError'?'timeout':'network',message:error?.name==='AbortError'?'انتهت مهلة الاتصال بـ Supabase.':'تعذر الوصول إلى Supabase؛ افحص الشبكة أو DNS.',error};
  }
}
