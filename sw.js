...
const CACHE_NAME='noor-v83-clean-release';
const CORE_ASSETS=[
  './',
  './index.html',
  './404.html',
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/config.js') || url.pathname.endsWith('/assets/js/v43-owner-portal.js') || url.pathname.endsWith('/assets/js/portal-loader.js') || url.pathname.endsWith('/supabase-check.html')){
    event.respondWith(fetch(new Request(event.request,{cache:'no-store'})));
    return;
  }

  // صفحات المصادقة/المالك network-first دائمًا لتجنب تشغيل نسخة قديمة من الكاش.
  const sensitive=/\/(?:portal|reset-password|supabase-check)\.html$/.test(url.pathname);
