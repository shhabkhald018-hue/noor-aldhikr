...
(() => {
  ...
  function explain(error){
    ...
    if(/PGRST205|42P01|schema cache|does not exist/i.test(raw)) return 'قاعدة البيانات ينقصها جزء من مخطط V82. شغّل ملف SUPABASE_V82_03_OWNER_RECOVERY.sql ثم أعد الاختبار.';
    ...
    if(/Failed to fetch|NetworkError|fetch/i.test(raw)) return 'تعذر الوصول إلى نطاق Supabase نفسه. هذا يشير غالبًا إلى DNS أو الشبكة أو حجب النطاق، وليس إلى role المالك.';
    if(/permission|row-level|RLS|42501/i.test(raw)) return 'الاتصال ناجح لكن RLS/الصلاحيات تمنع العملية. شغّل SUPABASE_V82_03_OWNER_RECOVERY.sql وتأكد أن الحساب Owner ونشط.';
    return raw;
  }
  ...
  async function probeSupabase(){
    ...
    try{
      // اختبار وصول شبكي فقط. mode=no-cors يمنع تشخيص CORS أو apikey خطأً على أنه DNS.
      await fetch(cfg.supabaseUrl.replace(/\/$/,'')+'/auth/v1/health',{method:'GET',mode:'no-cors',cache:'no-store',signal:ctrl.signal});
      clearTimeout(timer);
      return {ok:true,stage:'network',message:'تم الوصول إلى نطاق Supabase على مستوى الشبكة.'};
    }catch(error){
      clearTimeout(timer);
      if(error?.name==='AbortError') return {ok:false,stage:'timeout',message:'انتهت مهلة الوصول إلى Supabase. افحص الشبكة أو DNS.'};
      return {ok:false,stage:'network',message:'تعذر الوصول إلى نطاق Supabase على مستوى الشبكة. افحص DNS/الشبكة أو جرّب شبكة أخرى.',error};
    }
  }
  function initClient(){
    if(!validPortalConfig()) throw new Error('إعدادات Supabase غير مكتملة أو غير صحيحة في config.js.');
  ...
  async function refreshAll(initialWarning=''){
    ...
    if(errors.length){$('health').className='error';$('health').innerHTML=`هناك ${errors.length} مشكلة في ربط قاعدة البيانات:<ul>${errors.map(x=>`<li><b>${esc(x.label)}:</b> ${esc(explain(x.error))}</li>`).join('')}</ul><p><b>الحل:</b> شغّل <code>SUPABASE_V82_03_OWNER_RECOVERY.sql</code> ثم أعد التشخيص. لا تُعِد تشغيل ملفات SQL القديمة.</p>`;}
  ...
  function bind(){
    $('login').addEventListener('submit',async ev=>{
      ev.preventDefault();
      const submit=ev.submitter||$('login').querySelector('button[type="submit"]');
      if(submit)submit.disabled=true;
      msg('loginMsg','جاري تسجيل الدخول الفعلي إلى Supabase…','ok');
      try{
        // لا نمنع تسجيل الدخول بسبب اختبار health؛ طلب Auth نفسه هو الاختبار الحقيقي.
        const {data,error}=await timeout(sb.auth.signInWithPassword({email:$('email').value.trim().toLowerCase(),password:$('password').value}),20000);
        if(error)throw error;
        if(!data?.session)throw new Error('لم يُرجع Supabase جلسة دخول صالحة.');
        connectionState='up';setDiag('✓ الاتصال والمصادقة مع Supabase نجحا. جاري فحص صلاحية Owner…','ok');
        await boot();
      }catch(err){
        connectionState=/Failed to fetch|NetworkError|fetch|NOOR_TIMEOUT/i.test(String(err?.message||err))?'down':connectionState;
        msg('loginMsg',explain(err));
  ...
  async function checkConnectionAndPrepare(){
    setDiag('جاري فحص رابط Supabase…','muted');
  ...
  async function init(){
    try{
      ...
      if(meta){try{meta.textContent=`${cfg.appVersion||'unknown'} · ${new URL(cfg.supabaseUrl).hostname}`}catch{meta.textContent=cfg.appVersion||'unknown'}}
      $('portalRetry')?.addEventListener('click',async()=>{
        const networkOk=await checkConnectionAndPrepare();
        // اختبر API الحقيقي دائمًا؛ فحص health تشخيصي فقط وقد يفشل بسبب سياسة متصفح/شبكة بينما API نفسه يعمل.
        try{
          const test=await timeout(sb.from('site_modules').select('module_key').limit(1),12000,'NOOR_TIMEOUT_POSTGREST');
          if(test.error)throw test.error;
          connectionState='up';
          setDiag(networkOk?'✓ الشبكة وSupabase API يعملان. يمكنك تسجيل الدخول الآن.':'✓ Supabase API يعمل فعليًا رغم فشل فحص health. يمكنك تسجيل الدخول الآن.','ok');
          const {data:{session}}=await timeout(sb.auth.getSession(),12000);
          if(session)await boot();
        }catch(err){
          const prefix=networkOk?'تم الوصول للنطاق، لكن اختبار Supabase API فشل: ':'فشل فحص الشبكة وكذلك اختبار Supabase API: ';
          setDiag(prefix+explain(err),'error');
        }
      });
      // الاختبار الاستباقي للتشخيص فقط؛ لا يحجب زر تسجيل الدخول.
      await checkConnectionAndPrepare();
      try{
        const {data:{session},error}=await timeout(sb.auth.getSession(),12000);
        if(error)throw error;
        if(session)await boot();
      }catch(err){
        console.warn('Owner session restore:',err);
        msg('loginMsg','');
      }
  ...
  async function loadLegacyCms(){
    ...
    $('cmsLessons').querySelectorAll('.cmsDel').forEach(b=>b.onclick=async()=>{if(!confirm('حذف الدرس؟'))return;const {error}=await sb.from('academy_lessons').delete().eq('id',b.dataset.id);if(error)alert(explain(error));else loadLegacyCms()});
  }
  document.addEventListener('DOMContentLoaded',()=>{
    $('refreshLearning')?.addEventListener('click',loadLearning);
    $('lessonCmsForm')?.addEventListener('submit',async ev=>{ev.preventDefault();const row={subject:$('cmsSubject').value,title:$('cmsTitle').value.trim(),summary:$('cmsSummary').value.trim(),body:$('cmsBody').value.trim(),reference_text:$('cmsRef').value.trim(),active:true};const {error}=await sb.from('academy_lessons').insert(row);msg('cmsMsg',error?explain(error):'تمت إضافة الدرس.',error?'error':'ok');if(!error){ev.target.reset();loadLegacyCms()}});
    document.querySelectorAll('[data-tab="learning"]').forEach(b=>b.addEventListener('click',loadLearning));
    document.querySelectorAll('[data-tab="academyCms"]').forEach(b=>b.addEventListener('click',loadLegacyCms));
  });

  const V82_SUBJECTS={creed:'العقيدة',fiqh:'الفقه',seerah:'السيرة النبوية',hadith:'الحديث'};
  const V82_STATUS={draft:'مسودة',review:'قيد المراجعة',approved:'تمت المراجعة',published:'منشور'};
  ...
  function v50FillLessonSelect(){
    ...
    s.innerHTML='<option value="">بدون ربط</option>'+v50Lessons.map(x=>`<option value="${x.id}">${esc(V82_SUBJECTS[x.subject]||x.subject)} — ${esc(x.title)}</option>`).join('');
  ...
  async function v50RlsCheck(){
    const h=$('v50RlsResult');if(!h)return;h.className='muted';h.textContent='جاري تشغيل تشخيص V83.0…';
    const primary=await safe('owner_portal_diagnostics',sb.rpc('owner_portal_diagnostics'),null);
    if(!primary.error){h.className='ok';h.textContent='✓ تشخيص V83.0 ناجح: '+JSON.stringify(primary.data);return;}
