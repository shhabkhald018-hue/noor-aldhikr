'use strict';
(() => {
  const $=id=>document.getElementById(id); let sb=null,selectedUser=null,profileCache=[],ownerUserId='';
  ...
  function explain(error){
    ...
    if(/PGRST205|42P01|schema cache|does not exist/i.test(raw)) return 'قاعدة البيانات غير مهيأة بالكامل. شغّل ملف SUPABASE_V81_01_SCHEMA.sql في Supabase SQL Editor ثم عيّن حساب المالك.';
    if(/Invalid login credentials/i.test(raw)) return 'البريد أو كلمة المرور غير صحيحة، أو الحساب لم يتم إنشاؤه في Supabase Auth.';
    if(/Email not confirmed/i.test(raw)) return 'يجب تأكيد البريد الإلكتروني أولًا من رسالة Supabase.';
    if(/Failed to fetch|NetworkError|fetch/i.test(raw)) return 'تعذر الاتصال بـ Supabase. افحص الإنترنت ورابط المشروع والمفتاح في config.js.';
    if(/permission|row-level|RLS|42501/i.test(raw)) return 'تم الاتصال بقاعدة البيانات لكن صلاحيات RLS تمنع العملية. شغّل مخطط V81 وتأكد أن role للحساب هو owner.';
    return raw;
  }
  function initClient(){
    if(!window.NOOR_CONFIG?.supabaseUrl||!window.NOOR_CONFIG?.publishableKey) throw new Error('إعدادات Supabase غير مكتملة في assets/js/config.js.');
    if(!window.supabase?.createClient) throw new Error('تعذر تحميل مكتبة Supabase من CDN.');
    sb=window.supabase.createClient(NOOR_CONFIG.supabaseUrl,NOOR_CONFIG.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});window.supabaseClient=sb;
  }
  async function getOwner(){
    ...
    const {data:p,error:profileErr}=await sb.from('profiles').select('email,display_name,role,active').eq('id',user.id).maybeSingle();
    return {ok:true,user,profile:p||null,warning:profileErr?explain(profileErr):''};
  }
  async function boot(){
    msg('loginMsg','جاري التحقق…','ok');
    try{
      const o=await getOwner();
      if(!o.ok){msg('loginMsg',o.reason);return;}
      $('ownerEmail').textContent=o.user.email||'';ownerUserId=o.user.id;window.adminUser=o.user;$('gate').hidden=true;$('app').hidden=false;
      await refreshAll(o.warning);window.dispatchEvent(new CustomEvent('noor:admin-ready',{detail:{user:o.user}}));
    }catch(err){msg('loginMsg',explain(err));}
  }
  async function safe(label,promise,fallback=[]){
    try{const {data,error}=await promise;if(error)throw error;return {label,data:data??fallback,error:null}}catch(error){return {label,data:fallback,error}}
  }
  async function refreshAll(initialWarning=''){
    ...
    if(errors.length){$('health').className='error';$('health').innerHTML=`هناك ${errors.length} مشكلة في ربط قاعدة البيانات:<ul>${errors.map(x=>`<li><b>${esc(x.label)}:</b> ${esc(explain(x.error))}</li>`).join('')}</ul><p><b>الحل:</b> افتح ملف <code>SUPABASE_V81_01_SCHEMA.sql</code> وشغّله في Supabase SQL Editor.</p>`;}
  ...
  function bind(){
    $('login').addEventListener('submit',async ev=>{ev.preventDefault();msg('loginMsg','جاري تسجيل الدخول…','ok');try{const {error}=await sb.auth.signInWithPassword({email:$('email').value.trim(),password:$('password').value});if(error)throw error;await boot()}catch(err){msg('loginMsg',explain(err))}});
  ...
  async function init(){try{initClient();bind();const {data:{session}}=await sb.auth.getSession();msg('portalDiag','✓ تم تحميل Supabase وإعدادات الاتصال.','ok');if(session)await boot();}catch(err){msg('loginMsg',explain(err));msg('portalDiag',explain(err));}}
  ...
  const V50_SUBJECTS={creed:'العقيدة',fiqh:'الفقه',seerah:'السيرة النبوية',hadith:'الحديث'};
  const V50_STATUS={draft:'مسودة',review:'قيد المراجعة',approved:'تمت المراجعة',published:'منشور'};
  ...
  function v50FillLessonSelect(){
    ...
    s.innerHTML='<option value="">بدون ربط</option>'+v50Lessons.map(x=>`<option value="${x.id}">${esc(V50_SUBJECTS[x.subject]||x.subject)} — ${esc(x.title)}</option>`).join('');
  ...
  async function v50RlsCheck(){
    const h=$('v50RlsResult');if(!h)return;h.textContent='جاري فحص صلاحيات Owner والجداول…';
    const {data,error}=await sb.rpc('owner_rls_diagnostics');
    if(error){h.className='error';h.textContent=explain(error);return}
    h.className='ok';h.innerHTML=`✓ حساب Owner يعمل. profiles=${data.profiles??0} · progress=${data.learning_progress??0} · exams=${data.exam_attempts??0} · lessons=${data.academy_lessons??0} · questions=${data.academy_questions??0} · messages=${data.private_messages??0}<br><small>هذا تشخيص حساب المالك. استخدم RLS_TEST_V50.sql لاختبار زائر وطالب بشكل منفصل.</small>`;
  }
  document.addEventListener('DOMContentLoaded',()=>{
