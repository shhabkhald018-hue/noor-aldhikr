...
function authErrorArabic(error){
  ...
  if(low.includes('network') || low.includes('fetch') || low.includes('noor_timeout')) return 'تعذر الوصول إلى Supabase أو انتهت مهلة الاتصال. افحص الشبكة أو DNS ثم أعد المحاولة.';
...
async function initAuth(){
  ...
  if(!validConfig() || !supabaseClient){
    setAuthGate(false);
    const notice=qs('#dbNotice');if(notice){notice.classList.remove('hidden');notice.textContent='المزايا السحابية غير جاهزة حاليًا. يمكنك تصفح المحتوى العام.'}
    return;
  }

  if(window.noorProbeSupabase){
    const probe=await window.noorProbeSupabase(8000);
    if(!probe.ok){
      // لا نحجب نموذج الدخول بسبب اختبار health؛ طلب Auth الفعلي قد يعطي تشخيصًا أدق.
      const notice=qs('#dbNotice');if(notice){notice.classList.remove('hidden');notice.textContent=probe.message+' سيظل المحتوى العام متاحًا، ويمكنك محاولة تسجيل الدخول لاختبار Auth مباشرة.'}
    }
  }

  try{
    const {data,error}=await (window.noorWithTimeout?window.noorWithTimeout(supabaseClient.auth.getSession(),12000):supabaseClient.auth.getSession());
...
qs('#authForm')?.addEventListener('submit',async event=>{
  ...
  try{
    ...
    if(mode==='signup'){
      ...
      const result=await (window.noorWithTimeout?window.noorWithTimeout(supabaseClient.auth.signUp({email,password,options:{data:{display_name:displayName}}}),18000):supabaseClient.auth.signUp({email,password,options:{data:{display_name:displayName}}}));
      ...
      const login=await (window.noorWithTimeout?window.noorWithTimeout(supabaseClient.auth.signInWithPassword({email,password}),18000):supabaseClient.auth.signInWithPassword({email,password}));
    ...
    const result=await (window.noorWithTimeout?window.noorWithTimeout(supabaseClient.auth.signInWithPassword({email,password}),18000):supabaseClient.auth.signInWithPassword({email,password}));
    if(result.error) throw result.error;
...
qs('#forgotPassword')?.addEventListener('click',async()=>{
  ...
  try{
    ...
    const {error}=await (window.noorWithTimeout?window.noorWithTimeout(supabaseClient.auth.resetPasswordForEmail(email,{redirectTo}),18000):supabaseClient.auth.resetPasswordForEmail(email,{redirectTo}));
